...
export class Ground extends gfx.Mesh3
{
    ...
    public reshapeGround(stroke2D: Stroke2D, groundStartPoint: gfx.Vector3,  groundEndPoint: gfx.Vector3, camera: gfx.Camera): void
    {
        ...
        const normPlane = gfx.Vector3.cross((gfx.Vector3.subtract(groundEndPoint, groundStartPoint)), new gfx.Vector3(0, 1, 0));

        const plane = new gfx.Plane3(groundStartPoint, normPlane);
        ...
        for (let i = 0; i < this.vertices.length; i++) {
            const distance = plane.distanceTo(this.vertices[i]);
            const wd = Math.max(0, 1 - (Math.pow(distance/5, 2)));
            const h = this.computeH(this.vertices[i], newVerts, plane);
